...
ls =list.files('/Volumes/One Touch/raw', full.names = T, recursive = T)
ls = ls[!grepl('original$', ls)]
i = 1
file = ls[i]
ok_path = '/Volumes/One Touch/photos'
bad_path = '/Volumes/One Touch/deadletter'

for(i in seq_along(ls)){
  file = ls[i]
  if(as.integer(get_original_time(file) - original_exif_date(file))<100){
    file.copy(file, file.path(ok_path, basename(file)))
  }else{
    file.copy(file, file.path(bad_path, basename(file)))
  }
}
